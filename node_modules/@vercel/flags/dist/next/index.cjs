...
async function getOverrides() {
  ...
  const cookie = _optionalChain([_headers.cookies.call(void 0, ), 'access', _ => _.get, 'call', _2 => _2("vercel-flag-overrides"), 'optionalAccess', _3 => _3.value]);
...
function unstable_flag(definition) {
  const flag = async (code, precomputeFlags, secret = process.env.FLAGS_SECRET) => {
    ...
    const headers = _headers.headers.call(void 0, );
    const cookies2 = _headers.cookies.call(void 0, );
    ...
    let decisionPromise;
    if ("defaultValue" in definition && definition.defaultValue !== void 0) {
      try {
        decisionPromise = definition.decide({ headers, cookies: cookies2 });
      } catch (e) {
